var popBox = new function() {
	...
	this.open = function(width, height, title, url, closed) {
		...
		var html = '<div class="win"><div class="mask-layer"></div><div class="window-panel"><iframe class="title-panel" frameborder="0" marginheight="0" marginwidth="0" scrolling="no"></iframe><div class="title"><h3></h3></div><a href="javascript:void(0)" onclick="popBox._close();" class="close-btn" title="关闭">×</a><iframe class="body-panel" frameborder="0" marginheight="0" marginwidth="0" scrolling="auto" src=""></iframe></div></div>';
		var jq = $(html);
		jq.find(".window-panel").height(popBox.height).width(popBox.width).css("margin-left", -popBox.width / 2).css("margin-top", -popBox.height);
	...
	function messageBox(html, title, message, type) {
		var jq = $(html);
		if(type == "toast") {
			jq.find(".window-panel").width(message.length * 20).css("margin-left", -message.length * 20 / 2).css("margin-top", -popBox.height / 2);
		} else {
			jq.find(".window-panel").width(popBox.width).css("margin-left", -popBox.width / 2).css("margin-top", -popBox.height / 2 - 36);
		}
		if(valempty(title)) {
